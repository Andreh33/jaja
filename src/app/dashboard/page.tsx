...
export default async function DashboardHome() {
  ...
  const services = (empresa?.serviciosContratados as string[] | null) || [];
    ...
    <div className="px-6 py-8 md:px-10 md:py-12">
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl glass p-6">
          ...
          <div className="mt-3 font-display text-3xl text-white">{services.length}/3</div>
          <div className="mt-3 flex gap-1.5">
            ...
            {services.length === 0 && <span className="text-xs text-white/40">Aún ninguno indicado</span>}
          </div>
        </div>
        <div className="rounded-2xl glass p-6">
        ...
        <div className="rounded-2xl glass p-6">
          ...
          <div className="mt-3">
            <span className="inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold" style={{ background: 'rgba(139,92,246,0.15)', color: 'var(--purple-300)', border: '1px solid rgba(139,92,246,0.3)' }}>
              <span className="relative inline-flex h-1.5 w-1.5"><span className="absolute inset-0 rounded-full bg-purple-400 animate-pulse-dot" /></span>
              {empresa ? 'En diseño' : 'Recopilando información'}
            </span>
          </div>
        </div>
      </div>
