...
export default function ExperimentArt({ kind }: { kind: ExperimentSlug }) {
  return <div className={`${styles.art} ${styles[kind === 'rank-rush' ? 'rankArt' : kind === 'rayos-x' ? 'xrayArt' : `${kind}Art`]}`} aria-hidden="true">
    {kind === 'rank-rush' ? <><div className={styles.searchArt}><span>mi negocio, arriba del todo <b>⌕</b></span><i /><strong><small>01</small> Tu próxima gran idea. <b>↗</b></strong><i /><i /></div><span className={styles.rankBadge}>DE INVISIBLE<br /><strong>A IMPARABLE.</strong></span></> : null}
