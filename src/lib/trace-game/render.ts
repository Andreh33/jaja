...
export function drawLatechBean(ctx: CanvasRenderingContext2D, x: number, y: number, phase: number, airborne: boolean) {
  ...
  color.addColorStop(0, '#c9a6ff'); color.addColorStop(.45, '#8b5cf6'); color.addColorStop(1, '#5b21b6');
  ...
  ctx.fillStyle = '#f97316'; ctx.beginPath(); ctx.roundRect(-17, -17, 34, 5, 2); ctx.fill(); ctx.restore();
}
...
export function renderTrace(ctx: CanvasRenderingContext2D, width: number, height: number, level: TraceLevel, run: TraceRun, overview = false, reduced = false) {
  ...
  ctx.fillStyle = '#0b0716'; ctx.fillRect(0, 0, width, height);
  ...
  ctx.strokeStyle = 'rgba(201,166,255,.07)'; ctx.lineWidth = 1;
  for (let x = Math.floor(camera / 80) * 80; x < camera + viewWidth; x += 80) { ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, 510); ctx.stroke(); }
  ctx.strokeStyle = 'rgba(201,166,255,.23)'; ctx.lineWidth = 2; ctx.setLineDash([5, 9]); ctx.beginPath();
  ...
  for (const [index, p] of level.platforms.entries()) {
    ...
    ctx.fillStyle = index % 5 === 0 ? '#f97316' : '#a78bfa'; ctx.beginPath(); ctx.roundRect(p.x, p.y, p.width, 5, 3); ctx.fill();
  }
  const end = level.platforms[level.platforms.length - 1];
  ctx.strokeStyle = '#fbbf24'; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(level.finish, end.y); ctx.lineTo(level.finish, end.y - 72); ctx.stroke();
  ctx.fillStyle = '#fbbf24'; ctx.beginPath(); ctx.moveTo(level.finish, end.y - 72); ctx.lineTo(level.finish + 28, end.y - 61); ctx.lineTo(level.finish, end.y - 50); ctx.fill();
...
export function renderTraceCard(ctx: CanvasRenderingContext2D, level: TraceLevel, seconds: number | null) {
  ctx.fillStyle = '#0b0716'; ctx.fillRect(0, 0, 1200, 630);
  ctx.fillStyle = '#c9a6ff'; ctx.font = '600 22px sans-serif'; ctx.fillText('LATECH LAB / TU TRAZO, UN MUNDO', 64, 75);
  ...
  ctx.fillStyle = '#f97316'; ctx.font = '600 22px sans-serif'; ctx.fillText('serviciosonlineweb.com/lab/trazo', 64, 582);
