...
Preparado el 10 de septiembre de 2026. Este documento no ejecuta ni autoriza cambios de producción. La preparación y el aislamiento de Preview están descritos en `preview-readiness.md`; aquí se define cómo publicar una versión validada y recuperarse sin restaurar por defecto una base antigua. **Actualización de alcance:** el usuario ha autorizado después la publicación directa en producción conservando la versión anterior en una rama, sin una Preview previa. El coordinador registra y ejecuta ese destino; `production-ranking-operation.md` concreta el respaldo y DDL mínimo necesario. Las comprobaciones locales previas no se presentan como QA remoto.
...
- Antes de actuar, registrar en la entrega: SHA final revisado, resultado de QA sobre esa misma versión, ID/URL/SHA del deployment de producción que se sustituirá y destino exacto de la base. Si se usa Preview, incluir su URL/ID y QA propio; en la publicación directa autorizada se deja explícito que esa comprobación remota no se realizó. El coordinador completa estas referencias; una rama con cambios locales no sustituye ese registro.
- Registrar la autorización recibida y su destino. La autorización posterior de publicación directa incluye el esquema aditivo necesario; no requiere pedirla de nuevo. La publicación de las 14 piezas editoriales se mantiene como una operación separada con snapshot y control de conflictos, dentro del alcance que confirme el coordinador. La preparación de Preview y los scripts de fixture por sí solos nunca autorizan copiar una base QA a producción.
- La definición actual de Drizzle añade únicamente `escape_runs`, `escape_results` y sus tres índices explícitos, además de los índices internos asociados a claves primarias. No cambia las columnas de posts, usuarios, pedidos o contactos. El archivo de aplicación acotado es `scripts/sql/escape-ranking-v2.sql`.
